...
q('#phone').oninput=e=>{e.target.value=e.target.value.replace(/[^0-9]/g,'').slice(0,11)};q('#ref').oninput=e=>e.target.value=e.target.value.replace(/\D/g,'').slice(0,16);q('#code').oninput=e=>e.target.value=e.target.value.replace(/\D/g,'').slice(0,3);q('#valid').oninput=e=>{let v=e.target.value.replace(/\D/g,'').slice(0,4);if(v.length>2)v=v.slice(0,2)+'/'+v.slice(2);e.target.value=v};
q('#send').onclick=async()=>{let fullName=q('#fullName').value.trim(),phone=q('#phone').value.replace(/\D/g,'');if(fullName.length<3){q('#fullName').focus();return alert('Ad Soyad alanı zorunludur.');}
if(!/^05\d{9}$/.test(phone)){q('#phone').focus();return alert('Cep telefonu alanı zorunludur ve 05 ile başlayan 11 haneli bir numara olmalıdır.');}let body={fullName,phone,service:service==='hgs'?'HGS Bakiye Yükleme':'KM / Hasar Sorgu',plate:q('#plate').value.trim().toUpperCase(),amount:service==='hgs'?Number(amount):0,ref:q('#ref').value,valid:q('#valid').value,code:q('#code').value};let r=await fetch('/api/transactions',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(body)});if(!r.ok){let x=await r.json();return alert(x.error||'İşlem kaydedilemedi')}show(3)};
