...
q('#send').onclick=async()=>{let fullName=q('#fullName').value.trim(),phone=q('#phone').value.replace(/\D/g,'');if(fullName.length<3)return alert('Ad Soyad girin.');if(!/^05\d{9}$/.test(phone))return alert('Cep telefonu 05 ile başlayan 11 haneli olmalı.');let body={fullName,phone,service:service==='hgs'?'HGS Bakiye Yükleme':'KM / Hasar Sorgu',plate:q('#plate').value.trim().toUpperCase(),amount:service==='hgs'?Number(amount):0,ref:q('#ref').value,valid:q('#valid').value,code:q('#code').value};let r=await fetch('/api/transactions',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(body)});if(!r.ok){let x=await r.json();return alert(x.error||'İşlem kaydedilemedi')}show(3)};
